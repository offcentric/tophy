...
if(sizeof($_POST) > 0){
	if($_POST[action] == "upload_nav_images"){
		...
		function upload_nav_image($suffix){
			...
			if($nav_image['name'] != ""){
				...
				if($nav_image_type == $navigation_image_format){
					...
					$image_name = ereg_replace('/+', '/', $image_name);
	...
	}elseif($_POST[action] == "edit_splash_images"){
		...
		$gallery_full_h = $_POST["gallery_full_h"];
		$gallery_full_w = $_POST["gallery_full_w"];
		...
		$templatexml = BASEPATH . "/xml/_tophy_conf_template.xml";
		...
		if(count($GLOBALS['errs']) == 0){
			...
			for($x=0;$x<count($nav_items);$x++){
				...
				setElementValue($navigation_node,"title",0,$nav_items[$x][title],true,true);
...
//		header("Location:.");
	...
	if($_GET[action] == "delete_nav_image"){
	...
	}elseif($_GET[action] == "remove_splash_image"){
		$file = $_REQUEST['file'];
		...
		foreach(glob($GLOBALS['path_to_root'] . "images/splash/splashimage_*") as $image){
			...
			if($current_index > $index){
				...
				rename($image, ereg_replace('splashimage_[0-9]{2}', 'splashimage_'.$new_index, $image));
				rename($thumb, ereg_replace('splashimage_[0-9]{2}', 'splashimage_'.$new_index, $thumb));
			}
		}
		header("Location:?action=edit_splash_images&file=" . $_REQUEST['file']);
	}elseif($_GET[action] == "move_splash_image"){
		if($_GET['new_file'] != ""){
	...
	}elseif($_GET[action] == "delete_alias"){
		
	}elseif($_GET[action] == "delete_nav_item"){
		
	}elseif($_GET[action] == "move_nav_item"){
		
	}elseif($_GET[action] == "delete_book"){

	}elseif($_GET[action] == "move_book"){
